...
each line terminated by a newline character, but not all files follow this
convention. When `jj` encounters a missing terminating newline character in a
conflict, it will add a `[noeol]` comment to the conflict markers to indicate
that the "end-of-line" (EOL) character was missing. If you don't care about
whether your file ends with a terminating newline character, you can generally
ignore this comment and resolve the conflict normally.
...
missing a terminating newline character. Therefore, a resolution of this
conflict could be "grapefruit" with a terminating newline character.
