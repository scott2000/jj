...
fn parse_conflict_marker_any_len(line: &[u8]) -> Option<ConflictMarker> {
...
/// markers which have a different length are ignored.
fn parse_conflict_marker(line: &[u8], expected_len: usize) -> Option<ConflictMarkerKind> {
...
/// Find the longest conflict marker in a file.
pub fn find_max_conflict_marker_len(file: &[u8]) -> usize {
    file.lines_with_terminator()
...
pub fn choose_conflict_marker_len<T: AsRef<[u8]>>(single_hunk: &Merge<T>) -> usize {
    let max_existing_marker_len = single_hunk
        .iter()
        .map(|term| find_max_conflict_marker_len(term.as_ref()))
        .max()
        .unwrap_or_default();
    ...
    if max_existing_marker_len < MIN_CONFLICT_MARKER_LEN {
        ...
        // they're longer by a visually noticeable amount.
...
///
/// All conflict markers in the file must have the expected length. Any conflict
/// markers with a different length will be ignored.
...
pub fn parse_conflict(
    ...
    for line in input.lines_with_terminator() {
        match parse_conflict_marker(line, conflict_marker_len) {
...
fn parse_conflict_hunk(input: &[u8], conflict_marker_len: usize) -> Merge<BString> {
    ...
    let initial_conflict_marker = input
        ...
        .next()
        .and_then(|line| parse_conflict_marker(line, conflict_marker_len));

    match initial_conflict_marker {
        // JJ-style conflicts must start with one of these 3 conflict marker lines
        Some(ConflictMarkerKind::Diff | ConflictMarkerKind::Remove | ConflictMarkerKind::Add) => {
...
pub async fn update_from_content(
    ...
    let materialized_marker_len =
        materialize_merge_result(&merge_hunk, conflict_marker_style, &mut old_content).unwrap();
    if content == old_content {
        return Ok(file_ids.clone());
    }

    let max_content_marker_len = find_max_conflict_marker_len(content);

    // If all conflict markers in the file content are shorter than the materialized
    // conflict markers, then the file is considered resolved.
    if max_content_marker_len < materialized_marker_len.unwrap_or(MIN_CONFLICT_MARKER_LEN) {
        let file_id = store.write_file(path, &mut &content[..]).await?;
        return Ok(Merge::normal(file_id));
    }

    // Only accept the longest conflict markers in the file, also requiring that
    // they are at least as long as the materialized conflict markers. We need to
    // allow conflict markers to be longer than the materialized conflict markers,
    // since the user could have removed the "fake" conflict markers from the file
    // while editing, in which case their existing conflict markers will be
    ...
    let (used_file_ids, hunks) = 'hunks: {
        ...
        if simplified_file_ids.num_sides() != file_ids.num_sides() {
            if let Some(hunks) = parse_conflict(content, file_ids.num_sides(), expected_marker_len)
            {
