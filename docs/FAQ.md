...
be referred to by their change ID, but they require a numeric suffix to
distinguish them (e.g. `b800` can also be referred to as `lnv/1` as shown in
the evolog).
...
to hidden commits, divergent commits also require a numeric suffix when using
the change ID to refer to them, so you can see that `b8004ab` is still shown
as `lnvvtr/1` in the output. This temporary divergence is okay and will be
